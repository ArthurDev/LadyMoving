...
<div id="homecontent-{$hook}" class="col-xs-12">
	{foreach from=$leomanagewidgets_datas item=datas}
		...
		{elseif $datas.task == 'carousel'}
			<div class="leo-{$datas.task}">
				{foreach from=$datas.contents item=content}
					<div id="blockleohighlightcarousel-{$content.id}" class="block products_block exclusive blockleohighlightcarousel highlighted ">
					<div class="block_content">	
						<div class="highlight-carousel">
							<div class="highlight-image">
								{if $datas.configs.display_title}
									<h3 class="title_block"><span>Produits les plus vendus</span></h3>
								{/if}
							</div>
		...
		{elseif $datas.task == 'tab'}
			<div class="leo-{$datas.task} block products_block exclusive blockleoproducttabs">
				{if $datas.configs.display_title}
