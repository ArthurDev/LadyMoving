			...
			<td width="{$width}%" class="ajax_block_product product-block comparison_infos">
				<div class="product-container">
					...
					<div class="product_meta">
						<h3 class="name"><a href="{$product->getLink()}" title="{$product->name|truncate:32:'...'|escape:'htmlall':'UTF-8'}">{$product->name|truncate:27:'...'|escape:'htmlall':'UTF-8'}</a></h3>
						<div class="description">{$product->description_short|strip_tags|truncate:60:'...'}</div>
					...
					<div class="buttons-action product_bottom">
						<a class="cmp_remove clearfix button" href="{$link->getPageLink('products-comparison', true)|escape:'html'}" rel="ajax_id_product_{$product->id}">{l s='Remove'}</a>
						...
						<p class="comparison clearfix_availability_statut availability">
